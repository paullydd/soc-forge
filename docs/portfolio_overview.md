...
- Test coverage around the most important moving pieces, at 1,622 tests
...
- Attack-path reconstruction (`reconstruct_case`) doesn't yet read every field the way real pipeline alerts are shaped: its scoring (`same src_ip`/`username`/`host`) expects those fields at the top level of each item, but real alerts nest them under `details`. A case can therefore be correctly grouped as one unit while its *reconstructed narrative* only shows some of that case's steps as connected, not all of them. Separate from case grouping (fixed above), and a good candidate for its own follow-up.
