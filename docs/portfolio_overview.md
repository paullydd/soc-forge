...
- Test coverage around the most important moving pieces, at 1,611 tests
...
- Correlation and case reconstruction are hand-wired per rule-ID pair, not automatic: an alert only joins a case narrative if an engineer has explicitly added a correlation rule linking its rule ID to another. Several rules, including all of the Linux/web rules above, aren't wired into any correlation rule yet - they generate real alerts but won't currently chain into a multi-stage case on their own.
