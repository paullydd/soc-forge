...
- Test coverage around the most important moving pieces, at 1,620 tests
...
- Correlation is hand-wired per rule-ID pair, not automatic: an alert only joins a case narrative if an engineer has explicitly added a correlation rule linking its rule ID to another. All of `SOCF-023`/`SOCF-024`/`SOCF-027`/`SOCF-028`/`SOCF-029`/`SOCF-030` are now wired into both correlation (`SOCF-CORR-015`/`016`/`017`, chaining web recon -> exposure -> SSH access -> Linux privesc) and attack-path reconstruction. However, case grouping (`build_cases`) groups purely by the literal `correlation_id` string a correlation rule stamped on an alert, with no merging across correlations that share a "bridge" alert - so a chain of pairwise correlations three or more hops long (like the one above) currently produces multiple related-but-separate cases instead of one unified case, even though every individual link fires correctly and the full lineage is fully traceable by following shared entities (ip/host/username) across them. Fixing that would mean a union-find-style merge pass over the shared tagging/dedup logic all correlation rules use - a real, valuable, but separate piece of work.
