...
- YAML-based detection rules with explicit MITRE ATT&CK mappings, covering 10 of 14 Enterprise tactics across 23 rules
...
- Regression-tested behavior across 1,460 tests covering rules, correlation, case handling, the durable Investigation model, graphs, exports, simulation, and both the terminal console and web workspace
...
- Test coverage around the most important moving pieces, at 1,460 tests
...
- Four ATT&CK tactics have no rule coverage at all: Command and Control, Exfiltration, Reconnaissance, and Resource Development.
- The hunt-detection engine (`soc_forge/hunts/`) has not had a content update in several release cycles and is due for a refresh.
