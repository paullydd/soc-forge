...
The compact top bar exposes the current page title, a bounded purpose statement, local-platform state, v3.6.2 release identity, and existing scenario/search/refresh controls. It does not duplicate primary navigation. Active navigation uses both styling and `aria-current="page"`.
