...
- SSH authorized_keys modification via auditd file-watch telemetry (Linux persistence)
- Sensitive Linux account/privilege file modification - passwd, shadow, sudoers, cron (persistence)
...
- `SOCF-028`: tar/bsdtar `--checkpoint-action` wildcard/option injection on Linux, reconstructed as Privilege Escalation (`T1053.003`, Scheduled Task/Job: Cron) and Execution (`T1059.004`, Command and Scripting Interpreter: Unix Shell). Linux/auditd twin of `SOCF-026` (ingested via `--format linux-auditd`), gated on auditd execve telemetry (pid/exe/process_name) instead of Windows Event ID 4688 / Sysmon Event ID 1. Requires an auditd execve audit rule to be configured (e.g. `-a always,exit -F arch=b64 -S execve -k proc_exec`), the same deployment reality as Sysmon needing to be installed on Windows. Does not include `SOCF-026`'s parent/child fallback branch, since a single auditd SYSCALL/EXECVE record pair resolves only the parent's numeric ppid, not its own process name.
- `SOCF-029`: many distinct 404 paths from one source address within a short window, reconstructed as Reconnaissance (`T1595.003`, Active Scanning: Wordlist Scanning). Reads normalized `nginx-access-log` events; a 20-distinct-URI-in-2-minutes aggregate threshold, conservative against normal 404 noise but well below a real content-discovery tool's request rate.
- `SOCF-030`: a successful (`200`) request to a path commonly associated with unintentionally exposed version control, environment, or backup/config files (`.git/`, `.env`, `.svn/`, `.htpasswd`, `id_rsa`, `.pem`, `.bak`, `.sql`, `.db`), reconstructed as Credential Access (`T1552.001`, Unsecured Credentials: Credentials In Files). A URL-pattern heuristic - a matching `200` confirms the path was reachable, not that the response body contained sensitive content. This is the rule that would have caught the reference CTF's `.git` exposure directly.
- `SOCF-031`: a write to an SSH `authorized_keys` file, reconstructed as Persistence (`T1098.004`, Account Manipulation: SSH Authorized Keys). Reads normalized `linux-auditd` file-watch events (auditd `SYSCALL`+`PATH` records from a `-w <path>` watch rule, not the `SYSCALL`+`EXECVE` process-exec shape `SOCF-028` reads) - requires an auditd watch rule configured on the relevant `.ssh` directories, the same deployment reality as `SOCF-028`. This is the rule that would have caught the reference CTF's actual final action: planting a root SSH key for durable access.
- `SOCF-032`: a write to `/etc/passwd`, `/etc/shadow`, `/etc/sudoers` (or `/etc/sudoers.d/*`), or a cron path (`/etc/cron.d/*` or `/etc/crontab`), reconstructed as Persistence (`T1098`, Account Manipulation). Same `linux-auditd` file-watch telemetry as `SOCF-031`; deliberately one broader rule across this whole surface rather than several narrower ones (e.g. a dedicated sudoers rule mapped to `T1548.003`) - a reasonable future split, not done here.

Resource Development is deliberately not covered: it's pre-compromise, attacker-side activity (infrastructure acquisition, capability development) that never touches a monitored endpoint, so there is no honest way to detect it from any telemetry SOC-Forge ingests. Reconnaissance is *mostly* in the same position - network-level scanning (e.g. a port scan against arbitrary hosts) touches no endpoint SOC-Forge monitors - but `SOCF-029` is a genuine exception: content-discovery/directory brute-forcing against a specific, monitored web application does show up in that application's own access log, which is exactly the telemetry `SOCF-029` reads. Exfiltration is also not covered - meaningful exfiltration detection needs network telemetry (destination, data volume) that SOC-Forge does not ingest; a process-level proxy would be too speculative to trust.

These process rules support native Windows Security Event ID 4688. Event ID 1 is accepted only when the event provider is `Microsoft-Windows-Sysmon`. `SOCF-027` and `SOCF-028` are built on Linux telemetry instead: `SOCF-027` reads normalized `linux-auth-log` events (sshd auth lines from `/var/log/auth.log` or a journald export); `SOCF-028` reads normalized `linux-auditd` events, requiring a paired `SYSCALL`+`EXECVE` audit record - plain cron/syslog logging alone only shows the pre-expansion command string and is not sufficient telemetry for this rule. `SOCF-031`/`SOCF-032` read a *different* shape of the same `linux-auditd` format: a paired `SYSCALL`+`PATH` record from a `-w <path>` auditd file-watch rule, not a process-exec event - neither requires the other's audit rule to be configured, and a box could reasonably have one without the other.
