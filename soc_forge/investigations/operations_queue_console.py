...
def render_queue_item(
    ...
    *,
    ...
    resolved = resolve_terminal_width(width)
    rows = (
        ("Queue ID", item.queue_item_id),
        ("Priority", render_badge("priority", item.priority, ansi=ansi)),
        ("Item Type", item.item_type),
        ("Investigation", item.investigation_id),
        ("Title", item.investigation_title),
        ("Source ID", item.source_id),
        ("Source Status", render_badge("status", item.source_status, ansi=ansi)),
...
def render_queue_items(
    items: Iterable[OperationsQueueItem],
    *,
    width: int | None = None,
    ansi: bool | None = None,
) -> str:
...
class OperationsQueueConsoleController:
    ...
    def run(self) -> None:
        while True:
            items = self.queue_service.list_queue()
            self.screen("OPERATIONS QUEUE")
            self.output(render_operations_queue(self.queue_service.summarize(items)))
            choice = self.input("\nSelect option: ").strip()
            if choice == "0":
                return
            filters = {
                "1": lambda item: True,
                "2": lambda item: item.item_type == "response_action",
                "3": lambda item: item.item_type == "uncovered_finding",
                "4": lambda item: item.priority in {"critical", "high"},
            }
    ...
    def _browse(self, items: tuple[OperationsQueueItem, ...]) -> None:
        self.screen("OPERATIONS QUEUE")
    ...
    def _open(self, item: OperationsQueueItem) -> None:
        ...
        if self.input("\nSelect option: ").strip() != "1":
            return
        current = self.workspace_service.get_investigation(item.investigation_id)
        if item.item_type == "response_action":
            self.response_action_controller.open_action(current, item.source_id)
        else:
            self.finding_controller.open_finding(current, item.source_id)
